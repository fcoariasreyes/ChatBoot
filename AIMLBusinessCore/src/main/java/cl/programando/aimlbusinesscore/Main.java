...
ELOQUA_PARAM_TIMESTAMP=SMS_TEMP_TEXT
ELOQUA_PARAM_SMS_MESSAGEID=SMS_TEMP_TEXT
ELOQUA_PARAM_SMS_STATUS=SMS_TEMP_TEXT

#Parametros asociados a SMS - InfoBIP
INFOBIP_URL=https://api.infobip.com/sms/1/text/single
INFOBIP_URL_REVIEW=https://api.infobip.com/sms/1/reports?messageId=
...
public class Main {
	...
	public static void main(String[] args) throws ParseException, IOException {
		...
		if (args==null||args.length==0) {
			...
			OEBCUtil.logInfo("Type Process "+typeProcess);
			OEBCUtil.logInfo("Path Process "+path);

		}else {
			OEBCUtil.logInfo("This process sintax is \"java -jar OracleEloquaBusinessCore.jar [TYPE_PROCESS=process|reviewProcess] [PATH properties]\"");
			System.exit(0);
		}
